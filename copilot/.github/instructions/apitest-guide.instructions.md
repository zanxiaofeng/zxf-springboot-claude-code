...
| `DatabaseVerifier` | `apitest/support/sql/DatabaseVerifier.java` | JDBC 直接查询验证 DB 状态（countUsers, userExists, findUserIdByUsername 等） |
...
- **MockFactory 方法**: `mock{Service}{Scenario}` — 如 `mockNotificationAccepted()`、`mockNotificationFailure()`
- **MockVerifier 方法**: `verify{Service}{Action}` — 如 `verifyNotificationCalled(count)`、`verifyNotificationCalledWith(params...)`
...
void testCreateUser() throws Exception {
    ...
    String expected = JsonLoader.load("test-data/user/post/created.json",
            Map.of("username", "new.user", "email", "new@example.com"));
